...
```
console.log is displaying what you want to output in the console
return is used in a function to find the result or the output of the function
```

**_What's the difference between defining and calling a function?_**

```
defining a function is giving the function a name and setting it's parameters
calling a function is used to get its respective output by using its arguments
```
...
```
the value of a variable that is either characters or numbers surrounded by quotations
```

**_Number_**

```
the value of a variable that is an integer not surrounded by quotations
```

**_Boolean_**

```
the output of a block of code that results in either true or false
```

**_Array_**

```
 a variable that is used to store values of different data types within square brackets and commas seperating each value within the array
```
